    ...
    if (self.chatBox.status == ICChatBoxStatusShowVideo) { // 录制视频状态
        if (self.delegate && [self.delegate respondsToSelector:@selector(chatBoxViewController:didChangeChatBoxHeight:)]) {
            [UIView animateWithDuration:0.3 animations:^{
                [self.delegate chatBoxViewController:self didChangeChatBoxHeight:HEIGHT_TABBAR];
...
- (ICChatBoxMoreView *)chatBoxMoreView
{
    if (nil == _chatBoxMoreView) {
        ...
        ICChatBoxMoreViewItem *videoItem = [ICChatBoxMoreViewItem createChatBoxMoreItemWithTitle:@"对讲"
                                 imageName:@"sharemore_sight"];
        ICChatBoxMoreViewItem *docItem   = [ICChatBoxMoreViewItem createChatBoxMoreItemWithTitle:@"位置共享" imageName:@"sharemore_location"];
        ICChatBoxMoreViewItem *voiceItem   = [ICChatBoxMoreViewItem createChatBoxMoreItemWithTitle:@"语音" imageName:@"sharemore_voiceinput"];
        [_chatBoxMoreView setItems:[[NSMutableArray alloc] initWithObjects:photosItem, takePictureItem, videoItem, docItem, voiceItem, nil]];
    }
...
//            }
            ...
            ctrl.takeBlock = ^(id item) {
                if ([item isKindOfClass:[NSURL class]]) {
                    ...
                    NSString *path = [NSString stringWithFormat:@"%@/%@.jpg", PicPath, [NSString currentName]];//Documents,Library,tmp
                    NSData *imageData = UIImageJPEGRepresentation(item, 1.0f);
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (strongSelf.delegate && [strongSelf.delegate respondsToSelector:@selector(imagesFromAlbum:)]) {
                            [strongSelf.delegate imagesFromAlbum:pathArray];
                        }
                    });
                }
            };
    ...
    } else if (itemType == ICChatBoxItemVideo) {//小视频-》改对讲了
        // 发起语音
        if (_delegate && [_delegate respondsToSelector:@selector(micCommunicationStart)]) {
            [_delegate micCommunicationStart];
        }
//        [self resignFirstResponder];
//
...
//        }
    } else if (itemType == ICChatBoxItemLocation) {
...
//        }];
        // 百度
//        CSLocationSendVC *vc = [[CSLocationSendVC alloc] init];
//
//        CSMainTabNavigation *nav = [[CSMainTabNavigation alloc] initWithRootViewController:vc];
//        [self presentViewController:nav animated:YES completion:^{
//
//        }];
//        [self presentViewController:nav animated:YES completion:nil];
        if (self.delegate && [self.delegate respondsToSelector:@selector(locationShare)]) {
            [self.delegate locationShare];
        }
    } else if (itemType == ICChatBoxItemVoiceCm) {
        // 发起语音
        if (_delegate && [_delegate respondsToSelector:@selector(voiceCommunicationStart)]) {
            [_delegate voiceCommunicationStart];
        }
    }
}

- (void)choicePicFromAlbum{
    // 从相册中选取
    ZLPhotoActionSheet *actionSheet = [[ZLPhotoActionSheet alloc] init];
    //设置照片最大选择数
    actionSheet.maxSelectCount = 9 - self.pictureArray.count;
    weakify(self);
    ...
    [actionSheet showPhotoLibraryWithSender:self lastSelectPhotoModels:self.lastSelectMoldels completion:^(NSArray<UIImage *> * _Nonnull selectPhotos, NSArray<ZLSelectPhotoModel *> * _Nonnull selectPhotoModels) {
        ...
        for (int i = 0; i < selectPhotos.count; i++) {
            NSString *path = [NSString stringWithFormat:@"%@/%@_%d.jpg", PicPath, [NSString currentName],i];//Documents,Library,tmp
            NSData *imageData = UIImageJPEGRepresentation(selectPhotos[i], 1.0f);
            BOOL result = [imageData writeToFile:path atomically:YES];
            if (!result) {
                // 写入临时文件失败
...
- (void)chatBox:(ICChatBox *)chatBox changeStatusForm:(ICChatBoxStatus)fromStatus to:(ICChatBoxStatus)toStatus
{
    if (toStatus == ICChatBoxStatusShowKeyboard) {  // 显示键盘
        ...
        if (fromStatus == ICChatBoxStatusShowVoice || fromStatus == ICChatBoxStatusNothing) {
            ...
            [UIView animateWithDuration:0.3 animations:^{
                if (self.delegate && [self.delegate respondsToSelector:@selector(chatBoxViewController:didChangeChatBoxHeight:)]) {
                    [self.delegate chatBoxViewController:self didChangeChatBoxHeight:HEIGHT_TABBAR + HEIGHT_CHATBOXVIEW];
                }
            }];
        } else {  // 表情高度变化
...
//    }
    [[ICRecordManager shareManager] startRecordingWithFileName:self.recordName completion:^(NSError *error) {
        if (error) {   // 加了录音权限的判断
            ...
            if ([self.delegate respondsToSelector:@selector(voiceDidStartRecording)]) {
                [self.delegate voiceDidStartRecording];
            }
        }
    }];
}
...
- (void)chatBoxDidStopRecordingVoice:(ICChatBox *)chatBox
{
    ...
    [[ICRecordManager shareManager] stopRecordingWithCompletion:^(NSString *recordPath) {
        if ([recordPath isEqualToString:shortRecord]) {
            if ([self.delegate respondsToSelector:@selector(voiceRecordSoShort)]) {
                [self.delegate voiceRecordSoShort];
            }
            [[ICRecordManager shareManager] removeCurrentRecordFile:weakSelf.recordName];
        } else {    // send voice message
            if (self.delegate && [self.delegate respondsToSelector:@selector(chatBoxViewController:sendVoiceMessage:)]) {
                [self.delegate chatBoxViewController:weakSelf sendVoiceMessage:recordPath];
            }
        }
    }];
}
