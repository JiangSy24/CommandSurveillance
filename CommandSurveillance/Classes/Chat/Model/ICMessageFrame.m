...
- (void)setModel:(ICMessageModel *)model
{
    ...
    CGFloat topViewH      = model.isChatGroup ? 15 : 3;
    CGFloat cellMinW      = 60;     // cell的最小宽度值,针对文本
    
    if (_bIsTimeLabelHave) {
        cellMargin = 30;
        _timeLabelF = CGRectMake(0, 5, CSScreenW / 2, 20);
    }
    ...
    if (model.isSender) {
        ...
        if ([model.message.type isEqualToString:TypeText]) { // 文字
            ...
            CGFloat x             = CGRectGetMinX(_bubbleViewF)+bubblePadding + 4;
            ...
            UIImage *image   = [UIImage imageWithContentsOfFile:model.mediaPath];//[UIImage imageWithContentsOfFile:[[ICMediaManager sharedManager] imagePathWithName:model.mediaPath.lastPathComponent]];
            if (image) {
        ...
        } else if ([model.message.type isEqualToString:TypeLocation]){//位置分享
...
//            _bubbleViewF           = CGRectMake(bubbleX, cellMargin+topViewH, bubbleSize.width, bubbleSize.height);
            ...
            _bubbleViewF = CGRectMake(CGRectGetMinX(_headImageViewF) - headToBubble - CSScreenW/2, cellMargin+topViewH, CSScreenW/2, CSScreenW/3);

            _picViewF              = CGRectMake(0, 0, CSScreenW/2, CSScreenW/3);
        ...
        _userNameF = CGRectMake(_bubbleViewF.origin.x, _bubbleViewF.origin.y - topViewH, _bubbleViewF.size.width, topViewH);
    } else {    // 接收者
        ...
        if ([model.message.type isEqualToString:TypeText]) {
            ...
            _chatLabelF   = CGRectMake(x - 4, cellMargin + bubblePadding + topViewH, chateLabelSize.width, chateLabelSize.height);
            ...
            _topViewF              = CGRectMake(x+arrowWidth, cellMargin, topViewSize.width, topViewSize.height);
            _picViewF              = CGRectMake(x, cellMargin+topViewH, bubbleSize.width, bubbleSize.height);
        } else if ([model.message.type isEqualToString:TypeLocation]) {
            CGSize bubbleSize = CGSizeMake(253, 95.0);
            _bubbleViewF = CGRectMake(CGRectGetMaxX(_headImageViewF)+headToBubble, cellMargin+topViewH, bubbleSize.width, bubbleSize.height);
            CGSize topViewSize     = CGSizeMake(bubbleSize.width-arrowWidth, topViewH);
            CGFloat x              = CGRectGetMinX(_bubbleViewF);
