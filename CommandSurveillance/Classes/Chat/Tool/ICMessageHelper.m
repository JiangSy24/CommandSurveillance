...
+ (ICMessageFrame *)createMessageFrame:(NSString *)type
              ...
              receivedSenderByYourself:(BOOL)receivedSenderByYourself
                       isHaveTimeLabel:(BOOL)bIsTimeLabel
{
    ...
    if ([type isEqualToString:TypeText]) {
        ...
        message.content = content;
    } else if ([type isEqualToString:TypeLocation]) {
        message.content = @"[位置]";//位置
    ...
    modelF.bIsTimeLabelHave = bIsTimeLabel;
    modelF.model = model;
    return modelF;
}
