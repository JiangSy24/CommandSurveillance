...
@interface MainViewController () <MAMapViewDelegate,MapPoiTableViewDelegate,AMapSearchDelegate,UISearchBarDelegate,SearchResultTableVCDelegate,UISearchControllerDelegate>
...
- (void)initTopView{
    ...
    CGFloat fltopHeight = rectStatus.size.height+ rectNav.size.height;
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CSScreenW, fltopHeight)];
    view.backgroundColor = CSColorZiSe;
    view.hidden = YES;
    [_searchController.view addSubview:view];
    _view = view;
}
...
//    LocationDetailVC *locationDetailVC = [[LocationDetailVC alloc] initWithLatitude:_tableView.selectedPoi.location.latitude longitude:_tableView.selectedPoi.location.longitude title:_tableView.selectedPoi.name position:_tableView.selectedPoi.address];
    ...
    UIImage *screenshotImage = [_mapView takeSnapshotInRect:CGRectMake(0, 0, _mapView.width, _mapView.height)];
    
    LocationModel *model = [[LocationModel alloc] init];
    model.latitude = _tableView.selectedPoi.location.latitude;
    model.longitude = _tableView.selectedPoi.location.longitude;
    model.name = _tableView.selectedPoi.name;
    model.address = _tableView.selectedPoi.address;
    model.snapImage = screenshotImage;
    if (self.delegate && [self.delegate respondsToSelector:@selector(sendLocation:)]) {
        [self.delegate sendLocation:model];
    }
    [self dismissViewControllerAnimated:YES completion:^{
