...
- (IBAction)btnPhotoLibrary_Click:(id)sender
{
    if (![self judgeIsHavePhotoAblumAuthority]) {
        ...
        photoBrowser.bIsPicCenter = self.bIsPicCenter;
        ...
        [photoBrowser setDoneBlock:^(NSArray<ZLSelectPhotoModel *> *selPhotoModels, BOOL isSelectOriginalPhoto) {
            ...
            if (strongSelf.bIsPicCenter) {
                // 搞一手
                [strongSelf requestSelPhotosCenter:strongPB];
            }else{
                [strongSelf requestSelPhotos:strongPB]; // 传出的选中图片
            }
        }];
...
- (void)requestSelPhotos:(UIViewController *)vc
{
    ...
    for (int i = 0; i < self.arraySelectPhotos.count; i++) {
        ...
        if (model.bIsHaveEditImage) {
            [photos replaceObjectAtIndex:i withObject:[self scaleImage:model.editImage]];
...
#pragma mark - 请求所选择图片、回调
- (void)requestSelPhotosCenter:(UIViewController *)vc
{
    ZLProgressHUD *hud = [[ZLProgressHUD alloc] init];
    [hud show];
    ...
    for (int i = 0; i < self.arraySelectPhotos.count; i++) {
        ...
        [strFullPath appendString:model.strPicName];
...
- (void)presentVC:(UIViewController *)vc
{
    ...
    [nav.navigationBar setBackgroundImage:[UIImage imageWithColor:CSColorZiSe] forBarMetrics:(UIBarMetricsDefault)];
    ...
    nav.navigationBar.translucent = NO;//半透明
//    nav.navigationBar.translucent = NO;
//    nav.previousStatusBarStyle = self.previousStatusBarStyle;
//    [nav.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName: [UIColor grayColor]}];
//    [nav.navigationBar setBackgroundImage:[UIImage imageNamed:@"daohanglanbeijing"] forBarMetrics:UIBarMetricsDefault];
