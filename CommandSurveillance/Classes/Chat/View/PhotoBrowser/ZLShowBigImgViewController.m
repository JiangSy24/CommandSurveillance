...
@interface ZLShowBigImgViewController () <UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, ZLBigImageCellDelegate>
{
    ...
    NSMutableArray<PicModel*> *_editPics;
}

@property (nonatomic, strong) UILabel *labPhotosBytes;

//@property (nonatomic, strong) PicModel *curModel;
...
- (void)viewDidLoad {
    ...
    _editPics = [NSMutableArray array];
...
- (void)initBottomView
{
    ...
    _btnOriginalPhoto.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    ...
    [_btnOriginalPhoto setTitleColor:[UIColor colorWithWhite:0.7 alpha:1] forState:UIControlStateDisabled];
...
//    _btnDone.layer.cornerRadius = 3.0f;
    [_btnDone setTitleColor:kRGB(22, 159, 232) forState:UIControlStateNormal];
...
#pragma mark - UIButton Actions 放大后的编辑
...
//    vc.imageInfo = self.arrayPics[_currentPage-1].picImage;
//    vc.delegate = self;
...
- (void)getDrawImage:(UIImage*)image{
    self.arrayPics[_currentPage-1].picImage = image;
    self.arrayPics[_currentPage-1].bCachingIsOk = YES;
    [_editPics addObject:self.arrayPics[_currentPage-1]];
    [_collectionView reloadData];
}

- (void)btnDone_Click:(UIButton *)btn
{
    if (self.arraySelectPhotos.count == 0 && !self.bIsPicCenter) {
    ...
    if (self.arraySelectPhotos.count == 0 && self.bIsPicCenter) {
        PicModel *picm = _arrayDataSources[_currentPage-1];

        ZLSelectPhotoModel *model = [[ZLSelectPhotoModel alloc] init];
        model.strPicName = picm.picName;
        [_arraySelectPhotos addObject:model];
    }
    
    if (self.btnDoneBlock) {

        [self reflishSelectArray];
        self.btnDoneBlock(self.arraySelectPhotos, self.isSelectOriginalPhoto);
    }
}

- (void)reflishSelectArray{
    for (ZLSelectPhotoModel *zlSel in self.arraySelectPhotos) {
        for (PicModel *edits in _editPics) {
            if (!self.bIsPicCenter && [zlSel.localIdentifier isEqualToString:edits.asset.localIdentifier]) {
                zlSel.bIsHaveEditImage = YES;
                zlSel.editImage = edits.picImage;
            }
            
            if (self.bIsPicCenter && [zlSel.strPicName isEqualToString:edits.picName]) {
                zlSel.bIsHaveEditImage = YES;
                zlSel.editImage = edits.picImage;
            }
        }
...
- (void)navRightBtn_Click:(UIButton *)btn
{
    ...
    if (self.bIsPicCenter) {
        PicModel *modelPic = _arrayDataSources[_currentPage-1];
        if (![self isHaveCurrentPageImage]) {
            [btn.layer addAnimation:GetBtnStatusChangedAnimation() forKey:nil];
            
            ZLSelectPhotoModel *model = [[ZLSelectPhotoModel alloc] init];
            model.editImage = modelPic.picImage;
            model.strPicName = modelPic.picName;
            [_arraySelectPhotos addObject:model];
        } else {
...
{
    if (self.bIsPicCenter) {
        PicModel *asset = _arrayDataSources[_currentPage-1];
        for (ZLSelectPhotoModel *model in _arraySelectPhotos) {
            if ([model.strPicName isEqualToString:asset.picName]) {
                return YES;
            }
        }
...
- (void)removeCurrentPageImage
{
    if (self.bIsPicCenter) {
        PicModel *picm = _arrayDataSources[_currentPage-1];
        for (ZLSelectPhotoModel *model in _arraySelectPhotos) {
            if ([model.strPicName isEqualToString:picm.picName]) {
...
- (void)sortPic{
    _arrayDataSources = [NSMutableArray array];
    if (self.shouldReverseAssets) {
        NSEnumerator *enumerator = [self.arrayPics reverseObjectEnumerator];
        id obj;
        while (obj = [enumerator nextObject]) {
            [_arrayDataSources addObject:obj];
        }
        ...
        [_arrayDataSources addObjectsFromArray:self.arrayPics];
        _currentPage = self.selectIndex + 1;
    }
    self.title = [NSString stringWithFormat:@"%ld/%ld", _currentPage, _arrayDataSources.count];
}
...
{
    int iii = indexPath.row;
    ZLBigImageCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ZLBigImageCell" forIndexPath:indexPath];
    cell.bIsPicCenter = self.bIsPicCenter;
    cell.picModel = self.arrayPics[indexPath.row];
    cell.delegate = self;
    
    if (self.bIsPicCenter) {
        _btnOriginalPhoto.enabled = YES;
    }else{
...
- (void)cachingIsOk:(BOOL)bIsOk{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.bIsPicCenter) {
            ...
            _btnOriginalPhoto.enabled = bIsOk;
            self.arrayPics[_currentPage-1].bCachingIsOk = bIsOk;
        }
    });
}
