...
- (void)setPicModel:(PicModel *)picModel{
    ...
    if (self.bIsPicCenter) {
        if (_picModel.bCachingIsOk) {
            self.imageView.image = _picModel.picImage;
        ...
        [strFullPath appendString:picModel.picName];
...
- (void)setAsset:(PHAsset *)asset
{
    ...
    if (self.picModel.bCachingIsOk) {
        self.imageView.image = self.picModel.picImage;
        [self resetSubviewSize];
        return;
    }
...
//        frame.size.height = size.height;
    ...
    BOOL bStatuts = [[ZLPhotoTool sharePhotoTool] judgeAssetisInLocalAblum:self.asset];
    if (bStatuts || self.bIsPicCenter) {
        self.picModel.picImage = image;
    }
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(cachingIsOk:)]) {
        [self.delegate cachingIsOk:[[ZLPhotoTool sharePhotoTool] judgeAssetisInLocalAblum:self.asset]];
    }
